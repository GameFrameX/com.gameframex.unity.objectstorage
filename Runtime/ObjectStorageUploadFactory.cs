...
namespace GameFrameX.ObjectStorage.Runtime
{
    ...
    public static class ObjectStorageUploadFactory
    {
        ...
        public static IObjectStorageUploadManager Create<T>(string accessKey, string secretKey, string bucketName) where T : IObjectStorageUploadManager
        {
            var manager = Activator.CreateInstance<T>();
            manager.Init(accessKey, secretKey, bucketName);
            return manager;
        }
    }
}